...
from .contrastive_loss import multi_pos_cross_entropy
...
class MemoryConstruct(nn.Module):
    def __init__(self, num_features=256, num_samples=23051):
        super(MemoryConstruct, self).__init__()
        self.num_features = num_features
        self.num_samples = num_samples
        self.register_buffer('features', torch.randn(num_samples, num_features))
        self.base_dir = 'datasets/mevis/train'
        self.idx2negative = json.load(open(os.path.join(self.base_dir, 'idx2negative.json')))
        self.idx2positive = json.load(open(os.path.join(self.base_dir, 'idx2positive.json')))
        self.num_contrastive_batch = 100

    def update(self, instance_id, reid_embed):
        instance_id = concat_all_gather(instance_id)
        reid_embed = concat_all_gather(reid_embed)
        self.features[instance_id] = 0.1 * self.features[instance_id] + 0.9 * reid_embed

    def get_training_samples(self, instance_id):
        idx_positive = self.idx2positive[str(instance_id)]
        distractor_list = self.idx2negative[str(instance_id)]

        all_instance_id = [i for i in range(self.num_samples)]
        all_instance_id = [x for x in all_instance_id if x not in idx_positive]

        positive_embedding = self.features[idx_positive]
        size_negative = self.num_contrastive_batch - len(idx_positive)
        if len(distractor_list) > 0:
            all_instance_id = [x for x in all_instance_id if x not in distractor_list]
            if size_negative - len(distractor_list) < 0:
                idxs_other = distractor_list[:size_negative]
            else:
                size_negative = size_negative - len(distractor_list)
                idxs_other = np.random.choice(all_instance_id, size=size_negative, replace=False).tolist()
                idxs_other += distractor_list
        else:
            idxs_other = np.random.choice(all_instance_id, size=size_negative, replace=False).tolist()

        negative_embedding = self.features[idxs_other]
        return positive_embedding.clone().detach(), negative_embedding.clone().detach()
...
class VitaSetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
        ...
        self.train_memory_bank = MemoryConstruct()
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        src_masks = src_masks.reshape(L*B, cQ, T, H, W) # [3, 10, 8, 88, 104]

        src_masks = src_masks[idx] # Nt x T x Hp x Wp [12, 8, 88, 104]
    ...
    def loss_contrastive_sim(
        self, outputs, targets, indices, num_masks
    ):
        reid_embed = outputs["pred_contrastive_embed"]
        reid_embed = reid_embed.flatten(0, 1)  # LB , cQ, C
        L, n, c = reid_embed.shape
        clip_src_idx = self._get_src_permutation_idx(indices)  # len = LB
        clip_batch_idx = clip_src_idx[0].to(device=reid_embed.device)
        is_clip_valid = clip_batch_idx == 2
        reid_embed_list = reid_embed[clip_src_idx][is_clip_valid]
        labels = torch.tensor([targets[0]["refer_id"]], device=reid_embed.device)

        image_feat = reid_embed_list.mean(keepdim=True, dim=0)
        self.train_memory_bank.update(labels, image_feat)
        loss_inter = []
        for anchor_embedding, sample_label in zip(image_feat, labels):
            positive_embedding, negative_embedding = self.train_memory_bank.get_training_samples(sample_label.cpu().numpy())
            num_positive = positive_embedding.shape[0]
            pos_neg_embedding = torch.cat([positive_embedding, negative_embedding], dim=0)
            pos_neg_label = pos_neg_embedding.new_zeros((pos_neg_embedding.shape[0],), dtype=torch.int64)  # noqa
            pos_neg_label[:num_positive] = 1.
            anchor_embedding = anchor_embedding.unsqueeze(0)
            dot_product = torch.mm(anchor_embedding, pos_neg_embedding.t())
            ...
            aux_cosine_similarity = torch.mm(aux_normalize_anchor_embedding, aux_normalize_pos_neg_embedding.t())
            loss_inter.append(multi_pos_cross_entropy(dot_product, aux_cosine_similarity, pos_neg_label.unsqueeze(0)))
        loss_con_sim = torch.mean(torch.stack(loss_inter))

        return {"loss_con_sim": loss_con_sim}
    ...
    def get_loss(
        ...
        loss_map = {
            ...
            'contrastive_sim': self.loss_contrastive_sim,
        }
    ...
    def forward(self, outputs, clip_targets, frame_targets, frame_indices=None, return_idx=False):
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    if loss == "fg_sim" or loss == "contrastive_sim":
                        continue
