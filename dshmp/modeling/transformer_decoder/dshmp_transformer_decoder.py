...
class DshmpMultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.cross_static = CrossAttentionLayer(d_model=hidden_dim,
                    nhead=nheads,
                    dropout=0.1,
                    normalize_before=pre_norm)
    ...
    def forward(self, x, mask_features, clip_mask_features, lang_feat, lang_mask, mask = None, static_feat=None):
        ...
        output_static = self.cross_static(output, static_feat.unsqueeze(1).repeat(1, bs, 1))

        output = output + 0.1 * output_static
        ...
        lang_feat_mean = lang_feat.repeat(bs, 1, 1).transpose(0, 1)
        lang_mask = lang_mask.repeat(bs, 1)

        for i in range(self.num_layers):
            level_index = i % self.num_feature_levels
            attn_mask[torch.where(attn_mask.sum(-1) == attn_mask.shape[-1])] = False
            # attention: cross-attention first
            src_level, lang_feat_fusion = self.fusion_layers[i](
                v=src[level_index],
                l=lang_feat_mean,
