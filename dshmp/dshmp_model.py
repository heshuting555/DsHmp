...
from .modeling.transformer_decoder.dshmp_decoder import DsHmpHierarchical
from transformers import BertModel, RobertaModel, RobertaTokenizerFast
import spacy


@META_ARCH_REGISTRY.register()
class DsHmp(nn.Module):
    """
    ...
    def train_model(self, batched_inputs, iterations):
        ...
        features = self.backbone(images.tensor, lang_feat_sentence.repeat(bs, 1, 1), lang_mask.repeat(bs, 1, 1))

        BT = len(images) # batch * Frames
        T = self.num_frames if self.training else BT
        B = BT // T
        lang_mask = lang_mask.squeeze(-1)
        outputs, frame_queries, mask_features = self.sem_seg_head(features, lang_feat_fusion, lang_mask, static_feat=static_feat)
        ...
        vita_outputs = self.vita_module(frame_queries, lang_feat_fusion, lang_mask, motion_feat)
    ...
    def inference(self, batched_inputs):
        ...
        for i in range(math.ceil(num_frames / self.test_run_chunk_size)):
            ...
            outputs, _frame_queries, _mask_features = self.sem_seg_head(features, lang_feat_fusion, lang_mask_, static_feat=static_feat)
        ...
        vita_outputs = self.vita_module(frame_queries, lang_feat_fusion, lang_mask_, motion_feat)
